  ...
  public static class ModelBuilderExtension
  {
    ...
    public static void ConfigIdentity(this ModelBuilder modelBuilder)
    {
      ...
      var appliedOnClrTypes = clrTypes
        ...
        .Where(x =>
          !clrTypes.Any(t => x.IsSubclassOf(t) && t.IsAssignableTo(typeof(IIdentity<Guid>)))
        )
        .ToArray();
      foreach (var clrType in appliedOnClrTypes)
      {
        modelBuilder.Entity(clrType).HasKey(nameof(IIdentity<Guid>.Id));
        modelBuilder.Entity(clrType).Property(nameof(IIdentity<Guid>.Id)).ValueGeneratedNever();
      }
    }
    ...
    {
      return [.. builder.Model.GetEntityTypes().Select(e => e.ClrType).Where(t => !t.IsAbstract)];
    }
  }
}
