...
namespace ETLPipelineTool.Infrastructure.Extractors.Decorators
{
  ...
  public class LoggingExtractorDecorator : IExtractor
  {
    ...
    private static readonly Histogram<double> _latencyHistogram = _meter.CreateHistogram<double>(
      "etl.extract.latency-ms"
    );

    public LoggingExtractorDecorator(IExtractor inner, ILogger<LoggingExtractorDecorator> logger)
    {
      _inner = inner;
      _logger = logger;
      ...
      _retryPolicy = Policy
        ...
        .WaitAndRetry(
          ...
          onRetry: (ex, delay, attempt, _) =>
            _logger.LogWarning(ex, "Retry {Attempt} after {Delay} due to error", attempt, delay)
        );
    }
    ...
    )
    {
      ...
      await foreach (var table in enumerated.WithCancellation(cancellationToken))
      {
        ...
        async IAsyncEnumerable<IDictionary<string, object>> WrappedRows()
        {
          ...
          _rowCounter.Add(rowCount, new KeyValuePair<string, object?>("table", table.TableName));
